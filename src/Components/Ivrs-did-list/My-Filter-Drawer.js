...
//         return didFilters.every((filter) => {
//           let { field, condition, value } = filter;
...
//             value = value.toUpperCase();
//           }
//           if (!item[field]) return false; // If field is missing in item, fail the condition.
...
//             value = value.toUpperCase();
//           }
//           if (!item[field]) return false; // If field is missing in item, skip.
...
//     // If no data matches, still show empty list but notify user
...
const MyFilterDrawer = ({ openDrawer, toggleDrawer }) => {
  ...
  const applyHandler = () => {
    ...
    if (filterLogic === "all") {
      // AND logic
      didFilters.forEach(({ field, condition, value }) => {
        if (field === "accontid" || field === "agtyp")
          value = parseInt(value, 10);
        if (field === "tspnm") value = value.toUpperCase();
      ...
      // OR logic
      filteredData = filteredData.filter((item) =>
        didFilters.some(({ field, condition, value }) => {
          if (field === "accontid" || field === "agtyp")
            value = parseInt(value, 10);
          if (field === "tspnm") value = value.toUpperCase();
    ...
    <Drawer anchor="right" open={openDrawer}>
      <Box p={2} width="520px">
        ...
        <Grid container spacing={3}>
            ...
            <FormControl sx={{ width: "100px" }} size="small">
              ...
              >
                ...
                <MenuItem
                  value="any"
                  sx={{
                    fontWeight: filterLogic === "any" ? "bold" : "normal",
                    color: filterLogic === "any" ? "#6a69ff" : "inherit",
                    fontSize: "14px",
                  }}
                >
          ...
          <Grid container spacing={1.8} ml={0.5} mt={1}>
            {didFilters.map((filter, index) => (
              <React.Fragment key={index}>
                <Grid item xs={0.5} mr={2}>
                  <IconButton
          ...
          <Grid container gap={2} m={2} mt={4} ml={3.7}>
            <Grid item xs={2}>
              <Button
                variant="contained"
                fullWidth
